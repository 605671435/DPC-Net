...
class KFoldDataset:
    ...
    def __init__(self,
        ...
        if isinstance(dataset, dict):
            dataset.setdefault('lazy_init', True)
    ...
    def full_init(self):
        ...
        if osp.isfile(self.dataset.ann_file):
            case_list = mmengine.list_from_file(
                self.dataset.ann_file, backend_args=self.dataset.backend_args)
        else:
            case_list = os.listdir(self.dataset.data_prefix['img_path'])
        case_list.sort()
        ...
        self.dataset.case_list = [case_list[index] for index in indices]
        self.dataset.full_init()
        self._metainfo.update(case_nums=self.dataset._metainfo['case_nums'])
    ...
    def __repr__(self):
        ...
        if self._fully_initialized:
            body.append(f'Number of cases: \t{len(self.dataset.case_list)}')
